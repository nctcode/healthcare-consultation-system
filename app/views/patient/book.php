...
document.getElementById('specialty_select').addEventListener('change', function() {
    ...
    fetch('<?= url('/api/doctors/by-specialty/') ?>' + encodeURIComponent(specialtyId))
        .then(r => r.json())
        .then(data => {
            docSelect.innerHTML = '<option value="">Chọn bác sĩ</option>';
            if (!data.doctors || data.doctors.length === 0) {
                docSelect.innerHTML = '<option value="">Không có bác sĩ phù hợp</option>';
                return;
            }
        ...
        .catch(() => {
            docSelect.innerHTML = '<option value="">Không tải được danh sách bác sĩ</option>';
        });
});
